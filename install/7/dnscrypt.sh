...
echo -e "\n---- installing dnscrypt-proxy to /usr/local/bin"
mv $TAR_DIR/dnscrypt-proxy /usr/local/bin/dnscrypt-proxy

rm -rf $TAR_DIR $FILENAME

echo -e "\n---- [IMPORTANT] update your MacOS network connection DNS entries to include only '127.0.0.1' to use DNS Crypt."
