...
* Terminal & Ghostty enhancements

  Improve terminal usability including mouse integration and clipboard sync for CLI emacs inside Ghostty.

#+BEGIN_SRC emacs-lisp
  ;; Enable mouse support in terminals (like Ghostty)
  (unless (display-graphic-p)
    (xterm-mouse-mode 1))

  ;; Share system clipboard (pbcopy/pbpaste) in terminal mode
  (unless (display-graphic-p)
